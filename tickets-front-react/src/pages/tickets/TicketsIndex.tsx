	...
	const ticketContent = (
		<table className="w-full">
			...
			<tbody className="bg-sky-100">
				{tickets.map(
					({ id, passengerName, passengerSSN, from, to, price, time }) => (
						<tr
							className="border-2 border-gray-300 h-12 hover:bg-sky-300 transition-all duration-200"
							key={id}>
							...
							<td className="text-center">
								...
								<Link
									className="bg-red-600 p-1 rounded-md mx-1"
									to={`/tickets/delete/${id}`}>
									Delete
								</Link>
							</td>
						</tr>
